...
cli.run = function() {
  ...
  var cachedir = path.join(getUserHome(), ".sushipm");
  var cache = path.join(cachedir, "scripts.json");
  ...
  function readScripts(){
    var json = fs.readFileSync(cache, "utf-8");
  ...
  //Fetch scripts.json
  var writer = fs.createWriteStream(cache);
  request(repo).pipe(writer);
  writer.on("finish", readScripts);
};
